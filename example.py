...
if __name__ == "__main__":
    ...
    """
    normals_map_out = normalSpeed.depth_normal(
        depth,
        fx,
        fy,
        k_size,
    ...
    normals_map_in = normalSpeed.depth_normal(
        depth, fx, fy, k_size, distance_threshold, difference_threshold, True
    )
    print("normals_map_in z mean:", normals_map_in[:, :, 2].mean())

    cv2.imwrite("examplePicture/normal_out.jpg", norm2bgr(normals_map_out))

    cv2.imwrite("examplePicture/normal_in.jpg", norm2bgr(normals_map_in))

    cv2.imwrite("examplePicture/depth_view.jpg", depth2show(depth))
